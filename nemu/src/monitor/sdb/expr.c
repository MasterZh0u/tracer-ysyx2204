...
enum {
  TK_NOTYPE = 0x41, TK_EQ, NUM, HEX, TK_UEQ, REG, DEREF, MINUS
      ...
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
        ...
        switch (rules[i].token_type) {
          ...
          case '-':
            ...
            } else if (tokens[nr_token - 1].type == ')' || tokens[nr_token - 1].type == NUM 
                      || tokens[nr_token - 1].type == HEX || tokens[nr_token - 1].type == REG){
              // It's normal type
            ...
            nr_token++;
            break;
          case TK_NOTYPE:  // space is ignored
            break;
          case NUM:
          case HEX:
          case REG:
